...
$consul_personal_req = MYSQLI_query($con, "SELECT * FROM personal_req INNER JOIN cargo_full ON personal_req.cargo_idcargo = cargo_full.idcargo WHERE idpersonal ='$idpersonal'");
...
if ($formacion_validacion == 'VALIDO') {
  if ($nomb_cargo_espec == 'PROFESIONAL DE LA SALUD') {
    ...
    if ($nivel_estudios == 'TITULADO') {
      ...
      if ($resultado_post) {
        $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "NO APTO: No cumple con el TITULO requerido.");
    ...
    if ($fil_cursos > 0) {
      ...
      while ($ar_cursos = mysqli_fetch_array($consul_cursos)) {
        ...
        $creditos = $ar_cursos['creditos'];
        $new_creditos = $creditos*16;
        if($horas>=$new_creditos){
            $horas;
        }else{
            $horas = $new_creditos;
        }
    ...
    if ($fil_expe > 0) {
      ...
      while ($ar_expe = mysqli_fetch_array($consul_expe)) {
        ...
        if ($idlugar_trabajo_gene == 1 or $idlugar_trabajo_gene == 2 or $idlugar_trabajo_gene == 3 or $idlugar_trabajo_gene == 4 or $idlugar_trabajo_gene == 5) {
          ...
          $dias_t = $dias_t + $dias;
        } elseif ($idlugar_trabajo_gene == 6 or $idlugar_trabajo_gene == 7 or $idlugar_trabajo_gene == 9 or $idlugar_trabajo_gene == 9 or $idlugar_trabajo_gene == 10 or $idlugar_trabajo_gene == 11) {
      ...
      if ($puntos_total_cursos >= 25) {
        $puntos_total_cursos = 25;
      }
    } else {
      $anios_4_total = 0;
      $meses_4_total = 0;
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      $sql = "INSERT INTO evaluacion_curri_cas (titulo_prof, titulo_espec, egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,punt_total_forma, 
      curso_max_tres, curso_tres_seis, curso_seis_nueve, curso_nueve_mas, cursillos, punt_total_cursos, idioma_compu, expe_laboral_cuatro, expe_laboral_tres, expe_laboral_uno, 
      punt_total_expe,puntaje_total_total) VALUES ('$puntaje_nivel_estu', '$puntaje_espec_acre', '$puntaje_espec_egre','$puntaje_maestria_acre','$puntaje_maestria_egre',
      '$puntaje_doc_acre','$puntaje_doc_egre','$puntos_total_form','$puntos_total_3_menos','$puntos_total_3_6','$puntos_total_6_9','$puntos_total_9_mas', '$puntaje_cursillos',
      '$puntos_total_cursos_idioma','$puntos_idioma_comp','$suma_4_total', '$suma_3_total', '$suma_1_total', '$puntos_total_expe','$puntaje_total')";
      ...
      if ($result) {
        $actualizar_post = "UPDATE detalle_convocatoria SET estado_eva_curri_cas='AGREGADO', id_eva_curri_cas='$id', estado_conv_cas ='$estado_convocatoria',
        observaciones_detalle_cas='$observaciones' WHERE iddetalle_convocatoria='$iddetalle_convocatoria'";
        $resultado_post = MYSQLI_query($con, $actualizar_post);
        if ($resultado_post) {
          $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
          'mensaje' => "Se calculó correctamente los puntos de la calificación curricular.");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        } else {
          $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
          'mensaje' => "No se pudo calcular correctamente los puntos de la calificación curricular.");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      } else {
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
        'mensaje' => "No se pudo agregar correctamente los valores en la tabla de evaluacion curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
      $id_eva_curri_cas = $ar_detalle_con['id_eva_curri_cas'];
      $sql = "UPDATE evaluacion_curri_cas SET titulo_prof = '$puntaje_nivel_estu', titulo_espec = '$puntaje_espec_acre', egre_espec = '$puntaje_espec_egre', 
      grado_maestria = '$puntaje_maestria_acre', egre_maestria = '$puntaje_maestria_egre', grado_doctorado = '$puntaje_doc_acre', egre_doctorado = '$puntaje_doc_egre',
      punt_total_forma = '$puntos_total_form', curso_max_tres = '$puntos_total_3_menos', curso_tres_seis = '$puntos_total_3_6', curso_seis_nueve = '$puntos_total_6_9', 
      curso_nueve_mas = '$puntos_total_9_mas', cursillos = '$puntaje_cursillos', punt_total_cursos = '$puntos_total_cursos_idioma', idioma_compu = '$puntos_idioma_comp', 
      expe_laboral_cuatro = '$suma_4_total', expe_laboral_tres = '$suma_3_total', expe_laboral_uno = '$suma_1_total', punt_total_expe = '$puntos_total_expe', 
      puntaje_total_total = '$puntaje_total' WHERE id_eva_curricular = '$id_eva_curri_cas'";
      $result = mysqli_query($con, $sql);
      if ($result) {
        $actualizar_post = "UPDATE detalle_convocatoria SET estado_conv_cas = '$estado_convocatoria',observaciones_detalle_cas = '$observaciones' 
        WHERE iddetalle_convocatoria='$iddetalle_convocatoria'";
        $resultado_post = MYSQLI_query($con, $actualizar_post);
        $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
        'mensaje' => "Se actualizó correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      } else {
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
        'mensaje' => "No se pudo actualizar correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    }
    ...
    if ($nivel_estudios == 'TITULADO') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
      'mensaje' => "No cumple con el TITULO o BACHILLER requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      if ($puntaje_nivel_estu == 45) {
        ...
        $sql = "INSERT INTO evaluacion_curri_cas (titulo_espec, egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,bachiller, punt_total_forma, 
        curso_max_tres, curso_tres_seis, curso_seis_nueve, curso_nueve_mas, cursillos, punt_total_cursos, idioma_compu, expe_laboral_cuatro, expe_laboral_tres, expe_laboral_uno, 
        punt_total_expe, puntaje_total_total) VALUES ('$puntaje_espec_acre', '$puntaje_espec_egre','$puntaje_maestria_acre','$puntaje_maestria_egre','$puntaje_doc_acre',
        '$puntaje_doc_egre','$puntaje_nivel_estu', '$puntos_total_form','$puntos_total_3_menos','$puntos_total_3_6','$puntos_total_6_9','$puntos_total_9_mas', 
        '$puntaje_cursillos','$puntos_total_cursos_idioma','$puntos_idioma_comp','$suma_4_total', '$suma_3_total', '$suma_1_total', '$puntos_total_expe','$puntaje_total')";
        $result = mysqli_query($con, $sql);
        //GUARDAR ULTIMO ID REGISTRADO
        $query = mysqli_query($con, "SELECT MAX(id_eva_curricular) AS id FROM evaluacion_curri_cas");
        if ($row = mysqli_fetch_row($query)) {
        ...
        if ($result) {
          ...
          $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
          'mensaje' => "No se pudo guardar los valores en la tabla de evaluacion curricular");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      }
    } else {
      ...
      if ($puntaje_nivel_estu == 45) {
        ...
        $sql = "UPDATE evaluacion_curri_cas SET titulo_espec = '$puntaje_espec_acre', egre_espec = '$puntaje_espec_egre', grado_maestria = '$puntaje_maestria_acre', 
        egre_maestria = '$puntaje_maestria_egre', grado_doctorado = '$puntaje_doc_acre', egre_doctorado = '$puntaje_doc_egre', bachiller = '$puntaje_nivel_estu', 
        punt_total_forma = '$puntos_total_form', curso_max_tres = '$puntos_total_3_menos', curso_tres_seis = '$puntos_total_3_6', curso_seis_nueve = '$puntos_total_6_9', 
        curso_nueve_mas = '$puntos_total_9_mas', cursillos = '$puntaje_cursillos', punt_total_cursos = '$puntos_total_cursos_idioma', idioma_compu = '$puntos_idioma_comp', 
        expe_laboral_cuatro = '$suma_4_total', expe_laboral_tres = '$suma_3_total', expe_laboral_uno = '$suma_1_total', punt_total_expe = '$puntos_total_expe', 
        puntaje_total_total = '$puntaje_total' WHERE id_eva_curricular = '$id_eva_curri_cas'";
    ...
    if ($nivel_estudios == 'BACHILLER') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
      'mensaje' => "No cumple con el BACHILLER requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    $estado_eva_curri_cas = $ar_detalle_con['estado_eva_curri_cas'];
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      $sql = "INSERT INTO evaluacion_curri_cas (titulo_espec, egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,bachiller,
      ...
      if ($result) {
        ...
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
        'mensaje' => "No se pudo agregar los datos a la tabla de evaluacion curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
      $id_eva_curri_cas = $ar_detalle_con['id_eva_curri_cas'];
      $sql = "UPDATE evaluacion_curri_cas SET titulo_prof = '$puntaje_nivel_estu', titulo_espec = '$puntaje_espec_acre', egre_espec = '$puntaje_espec_egre', 
      grado_maestria = '$puntaje_maestria_acre', egre_maestria = '$puntaje_maestria_egre', grado_doctorado = '$puntaje_doc_acre', egre_doctorado = '$puntaje_doc_egre',
      bachiller = '$puntaje_nivel_estu', punt_total_forma = '$puntos_total_form', curso_max_tres = '$puntos_total_3_menos', curso_tres_seis = '$puntos_total_3_6', 
      curso_seis_nueve = '$puntos_total_6_9', curso_nueve_mas = '$puntos_total_9_mas', cursillos = '$puntaje_cursillos', punt_total_cursos = '$puntos_total_cursos_idioma', 
      idioma_compu = '$puntos_idioma_comp', expe_laboral_cuatro = '$suma_4_total', expe_laboral_tres = '$suma_3_total', expe_laboral_uno = '$suma_1_total', 
      punt_total_expe = '$puntos_total_expe',puntaje_total_total = '$puntaje_total' WHERE id_eva_curricular = '$id_eva_curri_cas'";
      $result = mysqli_query($con, $sql);
      if ($result) {
        $actualizar_post = "UPDATE detalle_convocatoria SET estado_conv_cas = '$estado_convocatoria',observaciones_detalle_cas = '$observaciones' 
        WHERE iddetalle_convocatoria='$iddetalle_convocatoria'";
    ...
    if ($nivel_estudios == 'TITULADO') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 
      'mensaje' => "No cumple con el TITULO DE TÉCNICO requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    $estado_eva_curri_cas = $ar_detalle_con['estado_eva_curri_cas'];
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      $sql = "INSERT INTO evaluacion_curri_cas (titulo_espec, egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,titulo_tecno,punt_total_forma, curso_max_tres, curso_tres_seis, curso_seis_nueve, curso_nueve_mas, cursillos, punt_total_cursos, idioma_compu, expe_laboral_cuatro, expe_laboral_tres, expe_laboral_uno, punt_total_expe,puntaje_total_total) VALUES ('$puntaje_espec_acre', '$puntaje_espec_egre','$puntaje_maestria_acre','$puntaje_maestria_egre','$puntaje_doc_acre','$puntaje_doc_egre','$puntaje_nivel_estu','$puntos_total_form','$puntos_total_3_menos','$puntos_total_3_6','$puntos_total_6_9','$puntos_total_9_mas', '$puntaje_cursillos','$puntos_total_cursos_idioma','$puntos_idioma_comp','$suma_4_total', '$suma_3_total', '$suma_1_total', '$puntos_total_expe','$puntaje_total')";
      $result = mysqli_query($con, $sql);
      //GUARDAR ULTIMO ID REGISTRADO
      $query = mysqli_query($con, "SELECT MAX(id_eva_curricular) AS id FROM evaluacion_curri_cas");
      if ($row = mysqli_fetch_row($query)) {
      ...
      if ($result) {
        ...
        $resultado_post = MYSQLI_query($con, $actualizar_post);
        $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Se calculó correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      } else {
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo calcular correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
      $id_eva_curri_cas = $ar_detalle_con['id_eva_curri_cas'];
      ...
      if ($result) {
        ...
        $resultado_post = MYSQLI_query($con, $actualizar_post);
        $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Se actualizó correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      } else {
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo actualizar correctamente los puntos de la calificación curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    }
    ...
    if ($nivel_estudios == 'TITULADO') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No cumple con el TÍTULO TÉCNICO o EGRESADO requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      if ($puntaje_nivel_estu == 50) {
        ...
        if ($result) {
          ...
          $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al insertar los datos en la tabla evaluacion curricular cas.");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      } else {
      ...
      if ($puntaje_nivel_estu == 50) {
        ...
        if ($result) {
          ...
          $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo actualizó correctamente los puntos de la calificación curricular.");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      }
    }
    ...
    //FORMACION
    if ($tipo_estudios == 'SECUNDARIA COMPLETA') {
      $puntaje_nivel_estu = 50;
    } else {
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No cumple con SECUNDARIA COMPLETA requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    $estado_eva_curri_cas = $ar_detalle_con['estado_eva_curri_cas'];
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      $sql = "INSERT INTO evaluacion_curri_cas (titulo_espec,egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,sec_completa,punt_total_forma, curso_max_tres, curso_tres_seis, curso_seis_nueve, curso_nueve_mas, cursillos, punt_total_cursos, idioma_compu, expe_laboral_cuatro, expe_laboral_tres, expe_laboral_uno, punt_total_expe,puntaje_total_total) VALUES ('$puntaje_espec_acre','$puntaje_espec_egre','$puntaje_maestria_acre','$puntaje_maestria_egre','$puntaje_doc_acre','$puntaje_doc_egre','$puntaje_nivel_estu','$puntos_total_form','$puntos_total_3_menos','$puntos_total_3_6','$puntos_total_6_9','$puntos_total_9_mas', '$puntaje_cursillos','$puntos_total_cursos_idioma','$puntos_idioma_comp','$suma_4_total', '$suma_3_total', '$suma_1_total', '$puntos_total_expe','$puntaje_total')";
      $result = mysqli_query($con, $sql);
      //GUARDAR ULTIMO ID REGISTRADO
      $query = mysqli_query($con, "SELECT MAX(id_eva_curricular) AS id FROM evaluacion_curri_cas");
      if ($row = mysqli_fetch_row($query)) {
      ...
      if ($result) {
        ...
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al insertar datos de evaluacion curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
      $id_eva_curri_cas = $ar_detalle_con['id_eva_curri_cas'];
      ...
      if ($result) {
        ...
        if ($resultado_post) {
          ...
          $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo actualizar la tabla detalle convocatoria.");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      } else {
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo actualizar correctamente los puntos de la calificación curricular.");
  ...
  if ($result) {
    ...
    if ($result_detalle) {
      ...
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "El postulante quedó eliminado por no cumplir con la formación requerida.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    } else {
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo editar la tabla detalle convocatoria.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
  } else {
    $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Hubo un error al actualizar los datos.");
    echo json_encode($responder, JSON_UNESCAPED_UNICODE);
  }
}
