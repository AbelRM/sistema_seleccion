...
if ($formacion_validacion == 'VALIDO') {
  if ($nomb_cargo_espec == 'PROFESIONAL DE LA SALUD') {
    ...
    //EXPERIENCIA LABORAL
    $consul_expe = MYSQLI_query($con, "SELECT * FROM sistema_seleccion.expe_4puntos INNER JOIN sistema_seleccion.lugar_trabajo_gene 
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      $sql = "INSERT INTO evaluacion_curri_cas (titulo_prof, titulo_espec, egre_espec, grado_maestria, egre_maestria, grado_doctorado, egre_doctorado,punt_total_forma, curso_max_tres, curso_tres_seis, curso_seis_nueve, curso_nueve_mas, cursillos, punt_total_cursos, idioma_compu, expe_laboral_cuatro, expe_laboral_tres, expe_laboral_uno, punt_total_expe,puntaje_total_total) VALUES ('$puntaje_nivel_estu', '$puntaje_espec_acre', '$puntaje_espec_egre','$puntaje_maestria_acre','$puntaje_maestria_egre','$puntaje_doc_acre','$puntaje_doc_egre','$puntos_total_form','$puntos_total_3_menos','$puntos_total_3_6','$puntos_total_6_9','$puntos_total_9_mas', '$puntaje_cursillos','$puntos_total_cursos_idioma','$puntos_idioma_comp','$suma_4_total', '$suma_3_total', '$suma_1_total', '$puntos_total_expe','$puntaje_total')";
      ...
      if ($result) {
        ...
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo agregar correctamente los valores en la tabla de evaluacion curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      if ($puntaje_nivel_estu == 45) {
        ...
        if ($result) {
          ...
          $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo guardar los valores en la tabla de evaluacion curricular");
          echo json_encode($responder, JSON_UNESCAPED_UNICODE);
        }
      }
    } else {
    ...
    if ($nivel_estudios == 'BACHILLER') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No cumple con el BACHILLER requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
    ...
    if ($estado_eva_curri_cas == 'NO AGREGADO') {
      ...
      if ($result) {
        ...
        $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No se pudo agregar los datos a la tabla de evaluacion curricular.");
        echo json_encode($responder, JSON_UNESCAPED_UNICODE);
      }
    } else {
      $id_eva_curri_cas = $ar_detalle_con['id_eva_curri_cas'];
    ...
    if ($nivel_estudios == 'TITULADO') {
      ...
      $puntaje_nivel_estu = 0;
      $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "No cumple con el TITULO DE TÉCNICO requerido.");
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
    //POSTGRADO
    $consul_maestria = MYSQLI_query($con, "SELECT * FROM maestria_doc WHERE idpostulante_postulante ='$idpersonal' AND postgrado_validacion = 'VALIDO'");
