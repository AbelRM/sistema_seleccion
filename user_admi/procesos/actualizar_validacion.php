...
if (isset($_POST['updateCurso'])) {
  ...
  $sql = "UPDATE cursos_extra SET curso_validacion='" . $validacion . "',observaciones_selec='$observaciones',dni_update_curso_selec='" . $dato_desencriptado . "',fech_update_curso_selec = '$date' WHERE idcursos_extra='" . $idcursos . "' ";
  ...
  if ($result) {
    ...
    echo '<script> alert("Error al actualizar, verifique."); 
  ...
  $sql = "UPDATE formacion_acad_prac SET formacion_validacion='" . $validacion_formacion . "',observaciones_selec = '$observaciones',dni_update_formac_selec='" . $dato_desencriptado . "', fech_update_formac_selec = '$date' WHERE idformacion_acad_prac='" . $idformacion_acad_prac . "' ";
  ...
  $sql = "UPDATE maestria_doc SET postgrado_validacion='" . $postgrado_validacion . "', observaciones_selec='$observaciones', dni_update_post_selec='$dato_desencriptado', fech_update_post_selec = '$date' WHERE idmaestria_doc='" . $id_postgrado . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#lista_postgrado");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); 
          window.history.back(-1);</script>';
  }
  ...
  $validacion = $_POST['validacion_curso.'];
  $observaciones = $_POST['observaciones_curso'];
  ...
  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#$idcursos_extra");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); 
          window.history.back(-1);</script>';
  }
  ...
  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#lista_idiomas");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); 
          window.history.back(-1);</script>';
  }
  ...
  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#$id_expe4");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); 
          window.history.back(-1);</script>';
  }
