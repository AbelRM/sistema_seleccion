...
if (isset($_POST['editar'])) {
  ...
  if (!empty($_FILES['archivo']['name'])) {
    ...
    if (!(strpos($tipo_archivo, "pdf") && ($tamano_archivo <= 3000000))) {
      ...
      if (move_uploaded_file($_FILES['archivo_merito']['tmp_name'], $micarpeta . $nombre_archivo_merito)) {
        $sql = "UPDATE formacion_acad_prac SET tipo_estudios = '$tipo_estudios',nivel_estudios='$nivel_estudios_prof', centro_estudios = '$centro_estudios',ciclo_actual = '$ciclo_actual',carrera = '$carrera',fecha_inicio='$fecha_inicio',fecha_fin = '$fecha_fin',orden_merito='$orden_merito',archivo_merito='$nombre_archivo_merito' WHERE idformacion_acad_prac='$idformacion'";
        $result = mysqli_query($con, $sql);
        if ($result) {
          header('Location: ../formacion_prac.php?dni=' . $dni);
        } else {
    ...
    $sql = "UPDATE formacion_acad_prac SET tipo_estudios = '$tipo_estudios',nivel_estudios='$nivel_estudios_prof', centro_estudios = '$centro_estudios',ciclo_actual = '$ciclo_actual',carrera = '$carrera',fecha_inicio='$fecha_inicio',fecha_fin = '$fecha_fin',orden_merito='$orden_merito' WHERE idformacion_acad_prac='$idformacion'";
    $result = mysqli_query($con, $sql);
    if ($result) {
       header('Location: ../formacion_prac.php?dni=' . $dni);
    } else {
