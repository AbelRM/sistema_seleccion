...
$pdf->Ln(10);
$consulta6 = "SELECT * FROM postulante  where dni='$dato_desencriptado'";
...
while ($row = $resultado->fetch_assoc()) {
  ...
  $pdf->Cell(25, 10, $row['tipo'], 1, 1, 'C', 0);
}
...
$resultado = $con->query($consulta);
while ($row = $resultado->fetch_assoc()) {
  $pdf->Cell(50, 10, $row['nombre_general'], 1, 0, 'C', 0);
  $pdf->Cell(40, 10, $row['cargo'], 1, 0, 'C', 0);
  $pdf->Cell(40, 10, $row['fecha_inicio'], 1, 0, 'C', 0);
  $pdf->Cell(40, 10, $row['fecha_fin'], 1, 1, 'C', 0);
}
