...
if (isset($_POST['insertData'])) {
  ...
  if ($tipo_estudios == '1') {
    ...
    if (!(strpos($tipo_archivo, "pdf") && ($tamano_archivo <= 3000000))) {
      ...
      if (move_uploaded_file($_FILES['archivo']['tmp_name'], $micarpeta . $new_nombre)) {
        $sql = "INSERT INTO formacion_acad (tipo_estudios_id,nivel_estudios,centro_estudios,carrera,colegiatura,fecha_inicio,fecha_fin, formacion_idpostulante,archivo,brevete,serums,quintil,formacion_validacion) 
          VALUES('$tipo_estudios','$nivel_estudios_tec','$centro_estudios','$carrera','$colegiatura,'$fecha_inicio','$fecha_fin','$idpostulante','$new_nombre','$licencia_conducir','$serums','$serums','$formacion_validacion')";
        $result = mysqli_query($con, $sql);
        if ($result) {
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $micarpeta . $new_nombre)) {
          ...
          if ($nivel_estudios == 'TITULADO') {
            ...
            if ($tipo_prof == 'asistencial') {
              ...
              $sql = "INSERT INTO formacion_acad (tipo_estudios_id,nivel_estudios,centro_estudios,carrera,colegiatura,nro_colegiatura,fech_colegiatura,fech_habilitacion,fecha_inicio,fecha_fin, formacion_idpostulante,archivo, brevete, tipo_profesional,serums,quintil, valor_quintil,formacion_validacion) 
              VALUES('$tipo_estudios','$nivel_estudios','$centro_estudios', '$carrera','$colegiatura_validar','$nro_colegiatura','$fech_colegiatura','$fech_habilitacion','$fecha_inicio','$fecha_fin','$idpostulante','$new_nombre','$licencia_conducir','$tipo_prof','$serums','$serums','$quintil','$formacion_validacion')";
              ...
              $serums = $_POST['serums'];
              $sql = "INSERT INTO formacion_acad (tipo_estudios_id,nivel_estudios,centro_estudios,carrera,colegiatura,nro_colegiatura,fech_colegiatura,fech_habilitacion,fecha_inicio,fecha_fin, formacion_idpostulante,archivo, brevete, tipo_profesional,serums,quintil,formacion_validacion) 
  ...
  if ($orden_merito == 'NINGUNO') {
    $new_nombre_2 = '';
    if (move_uploaded_file($_FILES['archivo']['tmp_name'], $micarpeta . $new_nombre)) {
      $sql = "INSERT INTO formacion_acad_prac (tipo_estudios,nivel_estudios,ciclo_actual,centro_estudios,carrera,fecha_inicio,fecha_fin, formacion_acad_prac_idpostulante,archivo,orden_merito,archivo_merito,formacion_validacion) 
