 <?php
  ...
  WHERE idpracticas_conv ='$idpracticas' AND practicantel_req_idpracticantes_req = '$idpracticantes_req' AND estado_conv_prac ='APTO'");
  ...
  if ($num_consu > 0) {
    while ($datos2 = mysqli_fetch_array($consulta)) {
      ...
      $pdf->Cell(45, 6, $datos2['puntos_total_cv'], 1, 0, 'C');
      $pdf->Cell(50, 6, $datos2['observaciones_detalle_prac'], 1, 0, 'C');
      $pdf->Ln(6); //salto de linea
    }
  } else {
  ...
  $consulta = mysqli_query($con, "SELECT * FROM sistema_seleccion.detalle_conv_prac INNER JOIN sistema_seleccion.postulante 
  ON detalle_conv_prac.detalle_prac_idpostulante = postulante.idpostulante 
  WHERE detalle_conv_prac.idpracticas_conv ='$idpracticas' AND practicantel_req_idpracticantes_req = '$idpracticantes_req' AND estado_conv_prac='NO APTO'");
  $num_consu = mysqli_num_rows($consulta);
  $item = 0;
  if ($num_consu > 0) {
    while ($datos2 = mysqli_fetch_array($consulta)) {
