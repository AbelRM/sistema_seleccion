    ...
    if (!empty($_FILES['archivos1']['name'])) {
      ...
      if (!(strpos($tipo_archivo, "pdf") && ($tamano_archivo <= 3000000))) {
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $micarpeta . $nombre_archivo)) {
          ...
          $sql = "UPDATE formacion_acad SET tipo_estudios_id = '$tipo_estudios', centro_estudios = '$centro_estudios',colegiatura = '$colegiatura_validar',fecha_inicio='$fecha_inicio',fecha_fin = '$fecha_fin',archivo = '$nombre_archivo', brevete ='$licencia_conducir' WHERE id_formacion='$idformacion'";
      ...
      $serums = $_POST['serums'];
      $nivel_estudios = '';
      $sql = "UPDATE formacion_acad SET tipo_estudios_id = '$tipo_estudios', centro_estudios = '$centro_estudios',colegiatura = '$colegiatura_validar',fecha_inicio='$fecha_inicio',fecha_fin = '$fecha_fin', brevete ='$licencia_conducir' WHERE id_formacion='$idformacion'";
      $result = mysqli_query($con, $sql);
      if ($result) {
        echo '<script> alert("Guardado exitosamente"); </script>';
        header('Location: ../formacion.php?dni=' . $dni);
    ...
    if (!empty($_FILES['archivos1']['name'])) {
      ...
      if (!(strpos($tipo_archivo, "pdf") && ($tamano_archivo <= 3000000))) {
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $micarpeta . $new_nombre)) {
          $sql = "UPDATE formacion_acad SET tipo_estudios_id = '$tipo_estudios',nivel_estudios='$nivel_nivel_estudios_tecestudios', centro_estudios = '$centro_estudios',colegiatura = '$colegiatura_validar',fecha_inicio='$fecha_inicio',fecha_fin = '$fecha_fin', brevete ='$licencia_conducir' WHERE id_formacion='$idformacion'";
          $result = mysqli_query($con, $sql);
          if ($result) {
            echo '<script> alert("Guardado exitosamente"); </script>';
            header('Location: ../formacion.php?dni=' . $dni);
