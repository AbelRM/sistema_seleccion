...
$consulta = mysqli_query($con, "SELECT * FROM detalle_convocatoria INNER JOIN convocatoria 
ON detalle_convocatoria.convocatoria_idcon = convocatoria.idcon 
INNER JOIN personal_req ON detalle_convocatoria.personal_req_idpersonal = personal_req.idpersonal INNER JOIN cargo_full ON personal_req.cargo_idcargo = cargo_full.idcargo INNER JOIN ubicacion ON personal_req.personal_req_idubicacion = ubicacion.iddireccion
...
$pdf->Ln(10);
$pdf->Cell(190, 10, utf8_decode("RELACIÓN DE POSTULANTES APTOS PARA LA ENTREVISTA"), 0, 0, 'C');
$pdf->Ln(1); //salto de linea
$pdf->Cell(190, 10, '______________________________________________________', 0, 0, 'C');
$pdf->Ln(15);
$pdf->SetFont('Arial', 'B', 10);
...
//CONSULTA
//CONSULTA
$consulta = mysqli_query($con, "SELECT * FROM detalle_convocatoria INNER JOIN postulante 
...
while ($datos2 = mysqli_fetch_array($consulta)) {
  ...
  $pdf->Cell(50, 6, utf8_decode($datos2['observaciones_detalle_cas']), 1, 0, 'C');
  $pdf->Ln(6); //salto de linea
}
//FIN CONSULTA
$pdf->Ln(15); //SALTO DE LINEA
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(190, 10, utf8_decode("RELACIÓN DE POSTULANTES NO APTOS PARA LA ENTREVISTA"), 0, 0, 'C');
$pdf->Ln(1); //salto de linea
$pdf->Cell(190, 10, '__________________________________________________________', 0, 0, 'C');
$pdf->Ln(15);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(8, 12, utf8_decode("N°"), 'L,T,R', 0, 'C');
$pdf->Cell(85, 12, 'APELLIDOS Y NOMBRES', 'L,T,R', 0, 'C');
...
$consulta = mysqli_query($con, "SELECT * FROM detalle_convocatoria INNER JOIN postulante 
ON detalle_convocatoria.postulante_idpostulante = postulante.idpostulante INNER JOIN evaluacion_curri_cas ON detalle_convocatoria.id_eva_curri_cas = evaluacion_curri_cas.id_eva_curricular
WHERE detalle_convocatoria.convocatoria_idcon ='$idcon' AND personal_req_idpersonal = '$idpersonal' AND estado_conv_cas='NO APTO'");
