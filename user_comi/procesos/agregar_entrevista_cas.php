...
$estado_eva_curri_cas = $rw['estado_eva_curri_cas'];
...
if ($estado_eva_curri_cas == 'NO AGREGADO') {
  ...
  $sql = "INSERT INTO entrevista_cas (aspecto_personal, seguridad_estabilidad, capacidad_persu, capacidad_decisi, conocimiento_gen, puntaje_total) VALUES ('$aspec_perso', '$segu_estab', '$etica', '$compet', '$cono_cult', '$puntaje_entrevista')";
  ...
  if ($result) {
    $update = "UPDATE detalle_convocatoria SET estado_entrevista_cas='AGREGADO', id_entrevista_cas ='$id',dni_comision_update_entre='$dni', fech_update_comision_entre='$date' WHERE iddetalle_convocatoria = '$iddetalle_convocatoria'";
    ...
    if ($resultado) {
      ...
      if ($estado_resultado_final == 'NO AGREGADO') {
        ...
        $insert_resul_final = MYSQLI_query($con, "INSERT INTO resultado_final (puntaje_cv, porcentaje_cv, ponderado_cv, puntaje_entre, porcentaje_entre, ponderado_entre, total_puntaje_1,quintil_resul, quintil_porcen, valor_quintil,puntaje_total_quintil, discapacidad_porce, discapacidad_puntaje, servicio_mili_porce, servicio_mili, deportista_calif_porce, deportista_calif, puntaje_final_total) VALUES ('$puntaje_total_cv', '$porcen_eva_cu', '$new_puntaje_total_cv', '$puntaje_total_entre', '$porce_entrevista', '$new_puntaje_total_entre', '$puntaje_total_cv_entre', '$quintil','$serums','$puntaje_serums', '$puntaje_total_cv_entre_quinti','$porce_discapacidad', '$puntaje_discapacidad', '$porce_sermilitar', '$puntaje_militar', '$deportista_calificado', '$puntaje_deportista','$total_resultado_final')");
        ...
        if ($insert_resul_final) {
          ...
          if ($result) {
            $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Se agregó correctamente el puntaje de la entrevista con los resultados finales.");
            echo json_encode($responder, JSON_UNESCAPED_UNICODE);
          } else {
        ...
        $insert_resul_final = MYSQLI_query($con, "UPDATE resultado_final SET puntaje_cv='$puntaje_total_cv', porcentaje_cv='$porcen_eva_cu', ponderado_cv='$new_puntaje_total_cv', puntaje_entre='$puntaje_total_entre', porcentaje_entre='$porce_entrevista', ponderado_entre='$new_puntaje_total_entre', total_puntaje_1='$puntaje_total_cv_entre', quintil_resul='$quintil',quintil_porcen='$serums', valor_quintil='$puntaje_serums', puntaje_total_quintil = '$puntaje_total_cv_entre_quinti',discapacidad_porce='$porce_discapacidad', discapacidad_puntaje='$puntaje_discapacidad', servicio_mili_porce='$porce_sermilitar', servicio_mili='$puntaje_militar', deportista_calif_porce ='$deportista_calificado', deportista_calif='$puntaje_deportista',  puntaje_final_total='$total_resultado_final' WHERE idresultado_final = '$id_resultado_final'");

        if ($insert_resul_final) {
          $sql = "UPDATE detalle_convocatoria SET estado_resultado_final = 'AGREGADO', dni_comision_update_final='" . $dni . "', fech_update_comision_final ='" . $date . "' WHERE iddetalle_convocatoria = '$iddetalle_convocatoria' ";
          $result = MYSQLI_query($con, $sql);
      ...
      $responder =  array('r' => 0, 'dni' => $dni, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al agregar los datos en la tabla detalle convocatoria.", 'dni_postulante' => $iddetalle_convocatoria);
      echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    }
  } else {
    $responder =  array('r' => 0, 'dni' => $dni, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al insertar los datos en la tabla entrevista cas.", 'dni_postulante' => $dni_postulante);
    echo json_encode($responder, JSON_UNESCAPED_UNICODE);
  }
} else {
  ...
  $sql = "UPDATE entrevista_cas SET aspecto_personal='$aspec_perso', seguridad_estabilidad='$segu_estab', capacidad_persu='$etica', capacidad_decisi='$compet', conocimiento_gen='$cono_cult', puntaje_total='$puntaje_entrevista' WHERE id_entrevista_cas='$id_entrevista_cas'";
  $result = mysqli_query($con, $sql);
  ...
  if ($result) {
    ...
    $resultado = mysqli_query($con, $update);
    if ($resultado) {
      $consul_detalle_cas = MYSQLI_query($con, "SELECT * FROM detalle_convocatoria INNER JOIN convocatoria ON detalle_convocatoria.convocatoria_idcon = convocatoria.idcon INNER JOIN postulante ON detalle_convocatoria.postulante_idpostulante = postulante.idpostulante INNER JOIN evaluacion_curri_cas ON detalle_convocatoria.id_eva_curri_cas = evaluacion_curri_cas.id_eva_curricular INNER JOIN entrevista_cas ON detalle_convocatoria.id_entrevista_cas = entrevista_cas.id_entrevista_cas INNER JOIN formacion_acad ON formacion_acad.formacion_idpostulante = postulante.idpostulante WHERE iddetalle_convocatoria ='$iddetalle_convocatoria' AND estado_conv_cas = 'APTO'");
      $ar = mysqli_fetch_array($consul_detalle_cas);
      $estado_resultado_final = $ar['estado_resultado_final'];
      $porcen_eva_cu = $ar['porcen_eva_cu'];
      $porce_entrevista = $ar['porce_entrevista'];
      ...
      if ($estado_resultado_final == 'NO AGREGADO') {
        ...
        $total_resultado_final = $puntaje_total_cv_entre + $puntaje_discapacidad + $puntaje_militar + $puntaje_deportista + $puntaje_serums;
        //INSERTAR EN RESULTADO FINAL
        $insert_resul_final = MYSQLI_query($con, "UPDATE resultado_final SET puntaje_cv='$puntaje_total_cv', porcentaje_cv='$porcen_eva_cu', ponderado_cv='$new_puntaje_total_cv', puntaje_entre='$puntaje_total_entre', porcentaje_entre='$porce_entrevista', ponderado_entre='$new_puntaje_total_entre', total_puntaje_1='$puntaje_total_cv_entre', quintil_resul='$quintil', quintil_porcen='$serums',valor_quintil='$puntaje_serums', puntaje_total_quintil ='$puntaje_total_cv_entre_quinti', discapacidad_porce='$porce_discapacidad', discapacidad_porce='$puntaje_discapacidad', servicio_mili_porce='$porce_sermilitar', servicio_mili='$puntaje_militar', deportista_calif_porce ='$deportista_calificado', deportista_calif='$puntaje_deportista',  puntaje_final_total='$total_resultado_final' WHERE idresultado_final = '$id_resultado_final'");

        if ($insert_resul_final) {
          $sql = "UPDATE detalle_convocatoria SET estado_resultado_final = 'AGREGADO', dni_comision_update_final='" . $dni . "', fech_update_comision_final ='" . $date . "' WHERE iddetalle_convocatoria = '$iddetalle_convocatoria' ";
          $result = MYSQLI_query($con, $sql);
          if ($result) {
            $responder =  array('r' => 1, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Se actualizó correctamente el puntaje de la entrevista con los resultados finales.");
            echo json_encode($responder, JSON_UNESCAPED_UNICODE);
          } else {
            $responder =  array('r' => 0, 'dni' => $dni, 'idpostulante' => $idpostulante, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al actualizar la tabla detalle convocatoria.");
            echo json_encode($responder, JSON_UNESCAPED_UNICODE);
    ...
    $responder =  array('r' => 0, 'dni' => $dni, 'idcon' => $idcon, 'idpersonal' => $idpersonal, 'mensaje' => "Error al actualizar los datos en la tabla entrevista cas.", 'dni_postulante' => $dni_postulante);
    echo json_encode($responder, JSON_UNESCAPED_UNICODE);
  }
}
