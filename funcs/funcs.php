...
function isNull($nombres, $ape_pat, $ape_mat, $correo, $celular, $clave, $confi_clave, $tipo_documento)
{
  if (
    strlen(trim($nombres)) < 1 || strlen(trim($ape_pat)) < 1 || strlen(trim($ape_mat)) < 1
    || strlen(trim($correo)) < 1 || strlen(trim($celular)) < 1
    || strlen(trim($clave)) < 1 || strlen(trim($confi_clave)) < 1 || strlen(trim($tipo_documento)) < 1
  ) {
    return true;
  } else {
    return false;
...
function usuarioExiste($usuario)
{
  ...
  $stmt = $mysqli->prepare("SELECT iduser FROM usuarios WHERE usuario = ? LIMIT 1");
...
function emailExiste($email)
{
  ...
  $stmt = $mysqli->prepare("SELECT * FROM user WHERE correo = ? LIMIT 1");
...
function enviarEmail($email, $nombre, $asunto, $cuerpo)
{
  ...
  $mail->Username = 'selecciondiresa@amsoftperu.com'; //Modificar
  $mail->Password = 'bocasuelta123'; //Modificar

  $mail->setFrom('selecciondiresa@amsoftperu.com', 'Sistema de selección DIRESA TACNA'); //Modificar
