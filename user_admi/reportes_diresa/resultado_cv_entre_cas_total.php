 <?php
  ...
  INNER JOIN convocatoria ON detalle_convocatoria.convocatoria_idcon = convocatoria.idcon INNER JOIN ubicacion ON personal_req.personal_req_idubicacion = ubicacion.iddireccion INNER JOIN cargo_full ON personal_req.cargo_idcargo = cargo_full.idcargo
  ...
  $pdf->Cell(190, 10, '____________________________________________', 0, 0, 'C');
  $pdf->Ln(12); //SALTO DE LINEA
  $pdf->Cell(190, 10, utf8_decode("CONVOCATORIA N°" . ' ' . $arr['num_con'] . ' - ' . $arr['anio_con']), 0, 0, 'C');
  $pdf->Ln(1); //salto de linea
  $pdf->Cell(190, 10, '_________________________', 0, 0, 'C');
  $pdf->Ln(12); //SALTO DE LINEA

  $pdf->Cell(50, 5, utf8_decode("CARGO:"), 0, 0, '');
  ...
  if ($num_consu > 0) {
    while ($datos2 = mysqli_fetch_array($consulta)) {
      ...
      $pdf->Cell(65, 6, strtoupper($datos2['ape_pat'] . ' ' . $datos2['ape_mat'] . ', ' . $datos2['nombres']), 1, 0, '');
      $pdf->Cell(20, 6, $datos2['puntaje_total_total'], 1, 0, 'C');
      $pdf->Cell(25, 6, $datos2['estado_entrevista_cas'], 1, 0, 'C');
      $pdf->Cell(25, 6, $datos2['estado_conv_cas'], 1, 0, 'C');
      $pdf->Cell(45, 6, utf8_decode($datos2['observaciones_detalle_cas']), 1, 0, 'C');
      $pdf->Ln(6); //salto de linea
    }
  } else {
  ...
  $pdf->SetFont('Arial', 'B', 8);
  $pdf->Cell(190, 10, utf8_decode("NOTA: PUNTAJE MINIMO APROBATORIO 55 PUNTOS CURRICULAR"), 0, 0, '');
