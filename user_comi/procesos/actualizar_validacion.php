...
if (isset($_POST['updateCurso'])) {
  ...
  $sql = "UPDATE formacion_acad SET formacion_validacion='" . $validacion_formacion . "', observaciones_selec='$observaciones', dni_update_formac_comi	='$dato_desencriptado', 
  fech_update_formac_comi = '$date' WHERE id_formacion='" . $idformacion_cas . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#formacion_academica");
  } else {
    echo '<script> alert("Error al actualizar, verifique!"); </script>';
  }
  ...
  $sql = "UPDATE maestria_doc SET postgrado_validacion='" . $postgrado_validacion . "', observaciones_selec='$observaciones', dni_update_post_comi='$dato_desencriptado', 
  fech_update_post_comi = '$date' WHERE idmaestria_doc='" . $id_postgrado . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#tabla_postgrado");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); </script>';
  ...
  $sql = "UPDATE cursos_extra SET curso_validacion='" . $validacion . "', observaciones_selec='$observaciones', dni_update_curso_selec='$dato_desencriptado', 
  fech_update_curso_selec = '$date' WHERE idcursos_extra='" . $idcursos_extra . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#tabla_capacitaciones");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); </script>';
    echo "<script type=\"text/javascript\">history.go(-1);</script>";
  }
  ...
  $sql = "UPDATE idiomas_comp SET idioma_validacion='" . $validacion . "', observaciones_selec='$observaciones', dni_update_idioma_comi='$dato_desencriptado', 
  fech_update_idioma_comi = '$date' WHERE ididiomas_comp='" . $ididiomas_comp . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#tabla_idiomas");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); </script>';
    echo "<script type=\"text/javascript\">history.go(-1);</script>";
  }
  ...
  $sql = "UPDATE expe_4puntos SET expe_validacion='" . $validacion . "', observaciones_selec='$observaciones', dni_update_exp_comi='$dato_desencriptado', 
  fech_update_exp_comi = '$date' WHERE id_4puntos='" . $id_expe4 . "' ";

  $result = mysqli_query($con, $sql);

  if ($result) {
    header("Location: ../detalles_postulante_cas.php?idpostulante=$idpostulante&idcon=$idcon&idpersonal=$idpersonal&dni=$dato_desencriptado#tabla_experiencia");
  } else {
    echo '<script> alert("Error al actualizar, verifique."); </script>';
    echo "<script type=\"text/javascript\">history.go(-1);</script>";
  }
